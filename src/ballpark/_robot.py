...
class Robot:
    """Robot sphere decomposition with pre-computed analysis.

    Wraps a URDF and pre-computes expensive analysis (similarity detection,
    mesh distances) that can be reused across multiple spherization calls.

    Example:
        >>> robot = ballpark.Robot(urdf)
        >>> # Access pre-computed analysis
        >>> robot.links        # Links with collision geometry
        >>> robot.similarity   # SimilarityResult
        >>> # Allocate sphere budget
        >>> budgets = robot.allocate(target_spheres=100)
        >>> # Generate spheres
        >>> result = robot.spherize(target_spheres=100, refine=True)
        >>> # Or with explicit budgets
        >>> result = robot.spherize(budgets=budgets, refine=True)
    """
